...
/* ---------- Persistance (BLOC 1 : localStorage branché) ---------- */
...
const INITIAL_GOALS = [
  { id: 1, title: "Épargner 6 000€", category: "finance", owner: "both", deadline: "2026-12-31", progress: 35, status: "active", completedDate: null, motivA: 8, motivB: 9, notes: "Réduire abonnements inutiles", feelingA: "Motivé 💪", feelingB: "Confiante ✨", milestones: [{ id: 11, label: "Atteindre 1 000€", done: true }, { id: 12, label: "Atteindre 3 000€", done: false }, { id: 13, label: "Atteindre 6 000€", done: false }] },
  { id: 2, title: "Week-end à Lisbonne", category: "adventure", owner: "both", deadline: "2026-08-15", progress: 60, status: "active", completedDate: null, motivA: 9, motivB: 10, notes: "Regarder les vols en avril", feelingA: "Impatient 🌞", feelingB: "Excitée 🎉", milestones: [{ id: 21, label: "Réserver le vol", done: true }, { id: 22, label: "Réserver l'hôtel", done: false }] },
  { id: 3, title: "1 soirée sans écrans / semaine", category: "love", owner: "both", deadline: "2026-06-01", progress: 75, status: "active", completedDate: null, motivA: 7, motivB: 8, notes: "Jeux de société, cuisine ensemble", feelingA: "Bien 🙂", feelingB: "Heureuse 💕", milestones: [{ id: 31, label: "4 semaines consécutives", done: false }] },
  { id: 4, title: "Apprendre l'espagnol", category: "growth", owner: "B", deadline: "2026-12-31", progress: 20, status: "paused", completedDate: null, motivA: 6, motivB: 9, notes: "App Duolingo le soir", feelingA: "Hésitant 😅", feelingB: "Très motivée 🔥", milestones: [{ id: 41, label: "Terminer niveau A1", done: false }] },
];
        ...
        <div style={{ background: "#FAF5EE", borderRadius: 12, padding: "10px 14px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {goal.milestones.map(m => (
              <button key={m.id} onClick={() => onToggleMilestone(goal.id, m.id)} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", padding: 0, textAlign: "left" }}>
...
function GoalModal({ goal, settings, onSave, onClose }) {
  ...
  const addMilestone = () => setForm(f => ({ ...f, milestones: [...f.milestones, { id: Date.now(), label: "", done: false }] }));
  const updateMilestone = (id, label) => setForm(f => ({ ...f, milestones: f.milestones.map(m => m.id === id ? { ...m, label } : m) }));
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {form.milestones.map(m => (
                <div key={m.id} style={{ display: "flex", gap: 6 }}>
                  <input style={iStyle} value={m.label} onChange={e => updateMilestone(m.id, e.target.value)} placeholder="Ex: Atteindre 1 000€" />
                  <button onClick={() => removeMilestone(m.id)} aria-label="Retirer le jalon" style={{ background: "#FCEBEB", border: "none", borderRadius: 10, color: "#A32D2D", cursor: "pointer", padding: "0 12px", fontSize: 16 }}>−</button>
                </div>
              ))}
...
function SettingsModal({ settings, onSave, onClose, onExport }) {
...
export default function App() {
  const [goals, setGoals] = useLocalStorage("cg_goals", INITIAL_GOALS);
  const [reviews, setReviews] = useLocalStorage("cg_reviews", INITIAL_REVIEWS);
  const [settings, setSettings] = useLocalStorage("cg_settings", DEFAULT_SETTINGS);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FDF8F2", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #3D2C1E 0%, #6B3F2A 50%, #9B6B4A 100%)", padding: "40px 24px 50px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <button onClick={() => setSettingsModal(true)} aria-label="Réglages" style={{ position: "absolute", top: 20, right: 20, background: "rgba(255,248,238,0.12)", border: "1px solid rgba(255,248,238,0.2)", borderRadius: 12, color: "#FFF8EE", cursor: "pointer", padding: "8px 12px", fontSize: 16, zIndex: 2 }}>⚙️</button>
      ...
      {settingsModal && <SettingsModal settings={settings} onSave={(s) => { setSettings(s); setSettingsModal(false); }} onClose={() => setSettingsModal(false)} onExport={exportData} />}
